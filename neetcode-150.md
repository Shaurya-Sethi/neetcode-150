# Neetcode 150 (Section-wise)

Source: neetcode-gh/leetcode .problemSiteData.json

## Arrays & Hashing
- [x] LC0217 Contains Duplicate
- [ ] LC0242 Valid Anagram
- [ ] LC0001 Two Sum
- [ ] LC0049 Group Anagrams
- [ ] LC0347 Top K Frequent Elements
- [ ] LC0238 Product of Array Except Self
- [ ] LC0036 Valid Sudoku
- [ ] LC0271 Encode and Decode Strings
- [ ] LC0128 Longest Consecutive Sequence

## Two Pointers
- [ ] LC0125 Valid Palindrome
- [ ] LC0167 Two Sum II Input Array Is Sorted
- [ ] LC0015 3Sum
- [ ] LC0011 Container With Most Water
- [ ] LC0042 Trapping Rain Water

## Sliding Window
- [ ] LC0121 Best Time to Buy And Sell Stock
- [ ] LC0003 Longest Substring Without Repeating Characters
- [ ] LC0424 Longest Repeating Character Replacement
- [ ] LC0567 Permutation In String
- [ ] LC0076 Minimum Window Substring
- [ ] LC0239 Sliding Window Maximum

## Stack
- [ ] LC0020 Valid Parentheses
- [ ] LC0155 Min Stack
- [ ] LC0150 Evaluate Reverse Polish Notation
- [ ] LC0022 Generate Parentheses
- [ ] LC0739 Daily Temperatures
- [ ] LC0853 Car Fleet
- [ ] LC0084 Largest Rectangle In Histogram

## Binary Search
- [ ] LC0704 Binary Search
- [ ] LC0074 Search a 2D Matrix
- [ ] LC0875 Koko Eating Bananas
- [ ] LC0153 Find Minimum In Rotated Sorted Array
- [ ] LC0033 Search In Rotated Sorted Array
- [ ] LC0981 Time Based Key Value Store
- [ ] LC0004 Median of Two Sorted Arrays

## Linked List
- [ ] LC0206 Reverse Linked List
- [ ] LC0021 Merge Two Sorted Lists
- [ ] LC0143 Reorder List
- [ ] LC0019 Remove Nth Node From End of List
- [ ] LC0138 Copy List With Random Pointer
- [ ] LC0002 Add Two Numbers
- [ ] LC0141 Linked List Cycle
- [ ] LC0287 Find The Duplicate Number
- [ ] LC0146 LRU Cache
- [ ] LC0023 Merge K Sorted Lists
- [ ] LC0025 Reverse Nodes In K Group

## Trees
- [ ] LC0226 Invert Binary Tree
- [ ] LC0104 Maximum Depth of Binary Tree
- [ ] LC0543 Diameter of Binary Tree
- [ ] LC0110 Balanced Binary Tree
- [ ] LC0100 Same Tree
- [ ] LC0572 Subtree of Another Tree
- [ ] LC0235 Lowest Common Ancestor of a Binary Search Tree
- [ ] LC0102 Binary Tree Level Order Traversal
- [ ] LC0199 Binary Tree Right Side View
- [ ] LC1448 Count Good Nodes In Binary Tree
- [ ] LC0098 Validate Binary Search Tree
- [ ] LC0230 Kth Smallest Element In a Bst
- [ ] LC0105 Construct Binary Tree From Preorder And Inorder Traversal
- [ ] LC0124 Binary Tree Maximum Path Sum
- [ ] LC0297 Serialize And Deserialize Binary Tree

## Tries
- [ ] LC0208 Implement Trie Prefix Tree
- [ ] LC0211 Design Add And Search Words Data Structure
- [ ] LC0212 Word Search II

## Heap / Priority Queue
- [ ] LC0703 Kth Largest Element In a Stream
- [ ] LC1046 Last Stone Weight
- [ ] LC0973 K Closest Points to Origin
- [ ] LC0215 Kth Largest Element In An Array
- [ ] LC0621 Task Scheduler
- [ ] LC0355 Design Twitter
- [ ] LC0295 Find Median From Data Stream

## Backtracking
- [ ] LC0078 Subsets
- [ ] LC0039 Combination Sum
- [ ] LC0046 Permutations
- [ ] LC0090 Subsets II
- [ ] LC0040 Combination Sum II
- [ ] LC0079 Word Search
- [ ] LC0131 Palindrome Partitioning
- [ ] LC0017 Letter Combinations of a Phone Number
- [ ] LC0051 N Queens

## Graphs
- [ ] LC0200 Number of Islands
- [ ] LC0133 Clone Graph
- [ ] LC0695 Max Area of Island
- [ ] LC0417 Pacific Atlantic Water Flow
- [ ] LC0130 Surrounded Regions
- [ ] LC0994 Rotting Oranges
- [ ] LC0286 Walls And Gates
- [ ] LC0207 Course Schedule
- [ ] LC0210 Course Schedule II
- [ ] LC0684 Redundant Connection
- [ ] LC0323 Number of Connected Components In An Undirected Graph
- [ ] LC0261 Graph Valid Tree
- [ ] LC0127 Word Ladder

## Advanced Graphs
- [ ] LC0332 Reconstruct Itinerary
- [ ] LC1584 Min Cost to Connect All Points
- [ ] LC0743 Network Delay Time
- [ ] LC0778 Swim In Rising Water
- [ ] LC0269 Alien Dictionary
- [ ] LC0787 Cheapest Flights Within K Stops

## 1-D Dynamic Programming
- [ ] LC0070 Climbing Stairs
- [ ] LC0746 Min Cost Climbing Stairs
- [ ] LC0198 House Robber
- [ ] LC0213 House Robber II
- [ ] LC0005 Longest Palindromic Substring
- [ ] LC0647 Palindromic Substrings
- [ ] LC0091 Decode Ways
- [ ] LC0322 Coin Change
- [ ] LC0152 Maximum Product Subarray
- [ ] LC0139 Word Break
- [ ] LC0300 Longest Increasing Subsequence
- [ ] LC0416 Partition Equal Subset Sum

## 2-D Dynamic Programming
- [ ] LC0062 Unique Paths
- [ ] LC1143 Longest Common Subsequence
- [ ] LC0309 Best Time to Buy And Sell Stock With Cooldown
- [ ] LC0518 Coin Change II
- [ ] LC0494 Target Sum
- [ ] LC0097 Interleaving String
- [ ] LC0329 Longest Increasing Path In a Matrix
- [ ] LC0115 Distinct Subsequences
- [ ] LC0072 Edit Distance
- [ ] LC0312 Burst Balloons
- [ ] LC0010 Regular Expression Matching

## Greedy
- [ ] LC0053 Maximum Subarray
- [ ] LC0055 Jump Game
- [ ] LC0045 Jump Game II
- [ ] LC0134 Gas Station
- [ ] LC0846 Hand of Straights
- [ ] LC1899 Merge Triplets to Form Target Triplet
- [ ] LC0763 Partition Labels
- [ ] LC0678 Valid Parenthesis String

## Intervals
- [ ] LC0057 Insert Interval
- [ ] LC0056 Merge Intervals
- [ ] LC0435 Non Overlapping Intervals
- [ ] LC0252 Meeting Rooms
- [ ] LC0253 Meeting Rooms II
- [ ] LC1851 Minimum Interval to Include Each Query

## Math & Geometry
- [ ] LC0048 Rotate Image
- [ ] LC0054 Spiral Matrix
- [ ] LC0073 Set Matrix Zeroes
- [ ] LC0202 Happy Number
- [ ] LC0066 Plus One
- [ ] LC0050 Pow(x, n)
- [ ] LC0043 Multiply Strings
- [ ] LC2013 Detect Squares

## Bit Manipulation
- [ ] LC0136 Single Number
- [ ] LC0191 Number of 1 Bits
- [ ] LC0338 Counting Bits
- [ ] LC0190 Reverse Bits
- [ ] LC0268 Missing Number
- [ ] LC0371 Sum of Two Integers
- [ ] LC0007 Reverse Integer
